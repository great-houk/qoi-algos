...
std::vector<uint8_t> ReferenceQOI::encode(const std::vector<uint8_t>& data, QOIEncoderSpec& spec) {
    int p = 0, run = 0;
    ...
    if (spec.width == 0 || spec.height == 0 || spec.channels < 3 || spec.channels > 4 || spec.colorspace > 1 || spec.height >= QOI_PIXELS_MAX / spec.width) {
        return {};
    }

    int max_size = spec.width * spec.height * (spec.channels + 1) + QOI_HEADER_SIZE + sizeof(qoi_padding);
...
std::vector<uint8_t> ReferenceQOI::decode(const std::vector<uint8_t>& encoded_data, QOIDecoderSpec& spec) {
    ...
    if (spec.width == 0 || spec.height == 0 || spec.channels < 3 || spec.channels > 4 || spec.colorspace > 1 || header_magic != QOI_MAGIC || spec.height >= QOI_PIXELS_MAX / spec.width) {
        return {};
    }

// We're importing all needed modules including the header file that contains
// the class definitions
#include "qoi-cuda.hpp"
// This is the CUDA runtime API for memory management function like cudeMalloc,
// cudaMemcpy, cudaFree
#include <cuda_runtime.h>
// This includes definitions for CUDA Kernel launch parameters like threadIdx,
// blockIdx, blockDim
...
// We're executing the CUDA function call and we're storing the returned error
// code We're checking if the CUDA call failed (having any erorr code other than
// cudeSuccess)
...
__global__ void encode_segment_kernel(const uint8_t* pixels,
	...
	int end_px =
		(segment_idx == num_segments - 1) ? total_px : start_px + seg_px;
				...
				if (px_prev.v == INVALID_PIXEL) {
					...
					if (px.rgba.a == px_prev.rgba.a) {
						...
						if (vr > -3 && vr < 2 && vg > -3 && vg < 2 && vb > -3 &&
							vb < 2) {
							segment_out[out_pos++] = QOI_OP_DIFF |
													 (vr + 2) << 4 |
													 (vg + 2) << 2 | (vb + 2);
...
__global__ void decode_segment_kernel(const uint8_t* d_encoded,
									  uint8_t* d_pixels,
...
std::vector<uint8_t> CUDAQOI::encode(const std::vector<uint8_t>& pixels,
	...
	CUDA_CHECK(cudaMemcpy(d_pixels, pixels.data(), pixel_bytes,
						  cudaMemcpyHostToDevice));
	int threads_per_block = 1;
	int blocks = num_segments;

	encode_segment_kernel<<<blocks, threads_per_block>>>(
		d_pixels, d_output_segments, d_segment_sizes, channels, total_px,
		max_segment_size, seg_px, num_segments);

	CUDA_CHECK(cudaDeviceSynchronize());

	std::vector<int32_t> segment_size(num_segments);
	CUDA_CHECK(cudaMemcpy(segment_size.data(), d_segment_sizes,
						  num_segments * sizeof(int32_t),
	...
	for (int s = 0; s < num_segments; s++) {
		...
		CUDA_CHECK(cudaMemcpy(output.data() + offset,
							  d_output_segments + s * max_segment_size, size,
							  cudaMemcpyDeviceToHost));
	}
...
std::vector<uint8_t> CUDAQOI::decode(const std::vector<uint8_t>& encoded_data,
									 QOIDecoderSpec& spec) {
	QOIHeader header;
	...
	CUDA_CHECK(
		cudaMalloc(&d_checkpoints, num_checkpoints * sizeof(qoi_checkpoint_t)));
	CUDA_CHECK(cudaMalloc(&d_next_px_pos, num_checkpoints * sizeof(int)));
	CUDA_CHECK(cudaMemcpy(d_encoded, encoded_data.data(), encoded_data.size(),
						  cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_checkpoints, h_checkpoints.data(),
						  num_checkpoints * sizeof(qoi_checkpoint_t),
						  cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_next_px_pos, h_next_px_pos.data(),
						  num_checkpoints * sizeof(int),
						  cudaMemcpyHostToDevice));
	...
	decode_segment_kernel<<<blocks, threads_per_block>>>(
		d_encoded, d_pixels, d_checkpoints, d_next_px_pos, channels,
		chuncks_len);

	CUDA_CHECK(cudaDeviceSynchronize());
	std::vector<uint8_t> pixels(px_len);
	CUDA_CHECK(
		cudaMemcpy(pixels.data(), d_pixels, px_len, cudaMemcpyDeviceToHost));

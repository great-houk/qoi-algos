...
void* qoi_encode(const void* data, const qoi_desc* desc, int, int* out_len);
...
void* qoi_encode(const void* data,
        ...
        int max_checkpoints = (desc->width * desc->height * desc->channels + 1)/BYTES_PER_CHECKPOINT;
        unsigned int checkpoints[max_checkpoints];
        unsigned short byte_per_check_counter = 0;
        unsigned short checkpoint_counter = 0;

        // We need extra space for storing the checkpoints at the end, the extra end sequence after the checkps, and the end sequence for every single sequence end
        int checkpoint_padding = max_checkpoints * sizeof(qoi_padding) + max_checkpoints + sizeof(qoi_padding);
...
int qoi_write(const char* filename, const void* data, const qoi_desc* desc) {
        ...
        encoded = qoi_encode(data, desc, 10, &size);
        if (!encoded) {
                fclose(f);
                return 0;
        }

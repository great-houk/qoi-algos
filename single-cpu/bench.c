...
void libpng_encode_callback(png_structp png_ptr,
							png_bytep data,
							png_size_t length) {
...
void png_decode_callback(png_structp png, png_bytep data, png_size_t length) {
	...
	if (read_data->pos + length > read_data->size) {
		ERROR("PNG read %ld bytes at pos %d (size: %d)", length, read_data->pos,
			  read_data->size);
	}
...
void benchmark_print_result(benchmark_result_t res) {
	...
	for (int i = 0; i < BENCH_COUNT; ++i) {
		...
		printf("%s   %8.1f    %8.1f      %8.2f      %8.2f  %8ld   %4.1f%%\n",
			   lib_names[i], (double)res.libs[i].decode_time / 1000000.0,
			   (double)res.libs[i].encode_time / 1000000.0,
			   (res.libs[i].decode_time > 0
					? px / ((double)res.libs[i].decode_time / 1000.0)
					: 0),
			   (res.libs[i].encode_time > 0
					? px / ((double)res.libs[i].encode_time / 1000.0)
					: 0),
	...
	if (!opt_nodecode) {
		if (!opt_nopng) {
			BENCHMARK_FN(opt_nowarmup, opt_runs, res.libs[LIBPNG].decode_time, {
				int dec_w, dec_h;
				void* dec_p = libpng_decode(encoded_png, encoded_png_size,
											&dec_w, &dec_h);
				free(dec_p);
			});
			...
			BENCHMARK_FN(opt_nowarmup, opt_runs, res.libs[STBI].decode_time, {
				...
				void* dec_p =
					stbi_load_from_memory(encoded_png, encoded_png_size, &dec_w,
										  &dec_h, &dec_channels, 4);
				free(dec_p);
			});
		}
	...
	if (!opt_noencode) {
		if (!opt_nopng) {
			...
			BENCHMARK_FN(opt_nowarmup, opt_runs, res.libs[STBI].encode_time, {
				int enc_size = 0;
				stbi_write_png_to_func(stbi_write_callback, &enc_size, w, h,
									   channels, pixels, 0);
...
int main(int argc, char** argv) {
	...
	for (int i = 3; i < argc; i++) {
		if (strcmp(argv[i], "--nowarmup") == 0) {
			opt_nowarmup = 1;
		} else if (strcmp(argv[i], "--nopng") == 0) {
			opt_nopng = 1;
		} else if (strcmp(argv[i], "--noverify") == 0) {
			opt_noverify = 1;
		} else if (strcmp(argv[i], "--noencode") == 0) {
			opt_noencode = 1;
		} else if (strcmp(argv[i], "--nodecode") == 0) {
			opt_nodecode = 1;
		} else if (strcmp(argv[i], "--norecurse") == 0) {
			opt_norecurse = 1;
		} else if (strcmp(argv[i], "--onlytotals") == 0) {
			opt_onlytotals = 1;
		} else {
